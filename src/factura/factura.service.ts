...
import { UpdateFacturaDto } from './dto/factura.dto/updateFactura.dto';
...
async addProductoToFactura(nroFactura:number,idProducto:number,cantidad:number):Promise<void>{
    ...
    async findFacturasByProducto(idProducto:number):Promise<Factura[]>{
     ...
     async update(nro_factura: number, dto: UpdateFacturaDto): Promise<Factura>{
        const factura = await this.facturaRepository.findOneBy({ nro_factura })
        if(!factura){
          throw new NotFoundException("La factura no existe")
        }

        if(dto.fecha !== undefined) factura.fecha = dto.fecha;
        if(dto.total_sin_iva !== undefined) factura.total_sin_iva = dto.total_sin_iva;
        if(dto.iva !== undefined) factura.iva = dto.iva;
        if(dto.total_con_iva !== undefined) factura.total_con_iva = dto.total_con_iva;
        if (dto.nro_cliente !== undefined) {
          const cliente = await this.clienteRepository.findOneBy({ id: dto.nro_cliente });
          if(!cliente){
